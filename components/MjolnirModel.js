...
export default function MjolnirModel({
	position = [0, 0, 0],
	scale = 1,
	rotation = [0, 0, 0],
}) {
	const groupRef = useRef();
	// Load the Mjolnir model from the 'thormjolnir' folder
	const { scene } = useGLTF("/thormjolnir/scene.gltf");
	...
	useEffect(() => {
		if (groupRef.current) {
			groupRef.current.userData = {
				interactable: true,
				name: "Thor's Hammer (Mjolnir, Undamaged by Hela)", // Set the name for interaction
			};
		}
	}, []);
...
useGLTF.preload("/thormjolnir/scene.gltf");
